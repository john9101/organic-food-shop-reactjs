    ...
    return (
        <div
            ...
            <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                <Link to={`/products/${item.product_id}`} className="shrink-0 md:order-1">
                ...
                <div className="w-full min-w-0 flex-1 space-y-2 md:order-2">
                    <Link to={`/products/${item.product_id}`} className="text-base font-medium text-gray-900 hover:text-green-600">{item.product_title}</Link>
                    <div className="flex items-center gap-4">
                        <Button type="button" onClick={() => handleRemove()}
                                className="inline-flex items-center gap-1 p-0 text-sm font-medium bg-transparent text-red-600 hover:underline hover:bg-transparent dark:text-red-500">
                            <TrashIcon/> Xóa
                        </Button>
                    </div>
                </div>
