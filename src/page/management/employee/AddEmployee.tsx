...
import {AddEmployeeRequest} from "@/type/request/user.request.ts";
import {yupResolver} from "@hookform/resolvers/yup";
import {addEmployeeSchema} from "@/schema/auth-account-user.schema.ts";
import {useAppDispatch, useAppSelector} from "@/redux/hook.ts";
import {addEmployee, resetAddedEmployee} from "@/redux/slice/user.slice.ts";
...
const AddEmployee = () => {
    ...
    const addedEmployee = user.employee.added
    ...
    useEffect(() => {
        if (addedEmployee) {
            dispatch(resetAddedEmployee())
            navigate(`/employee-management`);
        }
    }, [addedEmployee])

    const addEmployeeForm = useForm<AddEmployeeRequest>({
        resolver: yupResolver(addEmployeeSchema),
    })

    const onSubmitAddEmployeeForm = (body: AddEmployeeRequest) => {
        const promise = dispatch(addEmployee(body))
    ...
    const handleChangeAddressField = async (e: ChangeEvent<HTMLInputElement>, index: number) => {
        addEmployeeForm.setValue(`addresses.${index}.specificPlace`, e.target.value);
    ...
    return (
        <Card className="grid gap-4 rounded-none border-none shadow-none">
            <CardHeader className="px-5">
                <CardTitle className="tracking-tighter text-green-600">Thêm mới nhân viên</CardTitle>
                <CardDescription>Nhập thông tin về nhân viên</CardDescription>
            </CardHeader>
            <Form {...addEmployeeForm}>
                <form onSubmit={addEmployeeForm.handleSubmit(onSubmitAddEmployeeForm)} className="space-y-6">
                    <CardContent>
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem className="flex flex-col items-start">
                                        <FormLabel className="text-black">Lương <span
                                            className="text-red-600">*</span></FormLabel>
                                        <FormControl>
                            ...
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem className="flex flex-col items-start">
                                        <FormLabel className="text-black">Ngày nhận việc <span
                                            className="text-red-600">*</span></FormLabel>
                                        <Popover>
                                            <PopoverTrigger asChild>
                                                <FormControl>
                                                                                ...
                                                                                listItemPlaces ? listItemPlaces.map((itemPlace, index) => (
                                                                                    <div key={index}
                                                                                         ...
                                                                                         onClick={() => {
                                                                                             ...
                                                                                             addEmployeeForm.setValue(`addresses.${i}.province`, itemPlace.compound.province)
                                                                                             addEmployeeForm.setValue(`addresses.${i}.district`, itemPlace.compound.district)
                                                                                             addEmployeeForm.setValue(`addresses.${i}.commune`, itemPlace.compound.commune)
                                                                                             setOpenSelectionPlaceContents(prevState =>
                                                                                                 prevState.map((open, idx) => idx === index ? false : open)
                                                                                             );
                    ...
                    <CardFooter className="flex gap-2 justify-end">
                        ...
                        <Button variant="secondary" asChild className="hover:text-black">
                            <Link to="/employee-management">Hủy</Link>
                        </Button>
